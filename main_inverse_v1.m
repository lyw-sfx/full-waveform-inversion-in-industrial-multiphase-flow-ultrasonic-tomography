...
% 学习率（步长）设置 - 自适应初始值
lr_speed = 1e-3;           % 声速学习率初始值
lr_alpha = 1e-3;           % 衰减系数学习率初始值
lr_adaptive = true;        % 启用自适应学习率
...
% 联合正则化参数
beta_joint = 0;            % 联合正则化权重（第1步为0，从第2步开始计算）
joint_reg_adaptive = true; % 是否使用自适应权重
joint_reg_window = 3;      % 结构相似性计算窗口大小
use_joint_reg = false;     % 是否使用联合正则（第1步为false）

% 存储上一步的场域（用于计算相似性正则）
field_speed_prev = [];     % 上一步声速场
field_alpha_prev = [];     % 上一步衰减场
...
alpha_min_limit = alpha_water ;   % 衰减系数归一化目标最小值
alpha_max_limit = alpha_oil   ;   % 衰减系数归一化目标最大值
...
fprintf('========================================\n');
fprintf('    FWI 声速-衰减联合重构（同时更新）\n');
fprintf('========================================\n');
fprintf('网格尺寸: %d x %d\n', FW_N, FW_N);
fprintf('重建区域: %d x %d\n', rec_mesh_num, rec_mesh_num);
fprintf('传感器数: %d（发射）x %d（接收）\n', num_sensor, num_rec);
fprintf('最大迭代: %d\n', max_iter);
fprintf('学习率: 声速=%.2e, 衰减=%.2e (自适应: %s)\n', lr_speed, lr_alpha, iif(lr_adaptive, '是', '否'));
fprintf('TV权重: 声速=%.2e, 衰减=%.2e\n', beta_tv_speed, beta_tv_alpha);
fprintf('联合正则: 第1步禁用，第2步起启用 (自适应: %s)\n', iif(joint_reg_adaptive, '是', '否'));
fprintf('========================================\n\n');

%% ==================== 主迭代循环 ====================
% 同时更新策略：
% 每次迭代同时计算声速和衰减的梯度，同时更新两个参数
% 第1步：不计算相似性正则
% 第2步起：使用上一步更新后的场域计算相似性正则

while num_iter < max_iter
    
    fprintf('\n========== 第 %d 次迭代 ==========\n', num_iter + 1);
    fprintf('更新策略: 同时更新声速和衰减\n');
    if use_joint_reg
        fprintf('相似性正则: 启用 (使用上一步场域计算)\n');
    else
        fprintf('相似性正则: 禁用 (第1步不计算)\n');
    end
    ...
    %% 损失函数计算
    % 总损失 = 数据保真度 + TV正则(声速) + TV正则(衰减) + 联合正则(可选)
    ...
    % 联合正则项（从第2步开始计算）
    if use_joint_reg && ~isempty(field_speed_prev) && ~isempty(field_alpha_prev)
        % 使用上一步更新后的场域计算相似性
        [joint_reg, ssim_val] = compute_joint_regularization(field_speed_prev, field_alpha_prev, joint_reg_window);
        
        % 自适应调整联合正则权重
        if joint_reg_adaptive
            beta_joint = 1e-2 * (1 - ssim_val) * 10;
            beta_joint = max(min(beta_joint, 1e-1), 1e-3);
        end
    else
        % 第1步不计算联合正则
        joint_reg = 0;
    ...
    %% 自适应学习率调整
    if lr_adaptive && num_iter > 0
        % 根据梯度大小调整学习率
        grad_speed_norm = norm(grad_speed);
        grad_alpha_norm = norm(grad_alpha);
        
        % 如果梯度较大，减小学习率；如果梯度较小，增大学习率
        if grad_speed_norm > 1e6
            lr_speed = max(lr_speed * 0.8, 1e-5);
        elseif grad_speed_norm < 1e3
            lr_speed = min(lr_speed * 1.1, 1e-1);
        end
        
        if grad_alpha_norm > 1e6
            lr_alpha = max(lr_alpha * 0.8, 1e-6);
        elseif grad_alpha_norm < 1e3
            lr_alpha = min(lr_alpha * 1.1, 1e-2);
        end
        
        fprintf('自适应学习率: 声速=%.2e, 衰减=%.2e\n', lr_speed, lr_alpha);
    end
    ...
    % 更新声速
    [medium_rec, lr_speed, success_speed] = line_search_update_speed(...
    ...
    % 更新衰减系数
    [medium_rec, lr_alpha, success_alpha] = line_search_update_alpha(...
    ...
    % 保存当前场域供下一步计算相似性正则使用
    field_speed_prev = medium_rec.sound_speed(sa_index:INV_N-sa_index, sa_index:INV_N-sa_index);
    field_alpha_prev = medium_rec.alpha_coeff(sa_index:INV_N-sa_index, sa_index:INV_N-sa_index);
...
function [medium_rec, new_lr, success] = line_search_update_speed(...
    ...
    if beta_joint > 0
        ...
        grad_joint_matrix = zeros(size(field_org));
    end
